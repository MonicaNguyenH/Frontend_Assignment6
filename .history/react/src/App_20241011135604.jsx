...
function App() {
  ...
  const addTask = (task) => {
    setTasks([...tasks, { id: tasks.length, text: task,  completed: false}]); // add task incompleted by default
  };
  ...
  const removeTask = (taskId) => {
    setTasks(tasks.filter(task => task.id !== taskId)); // update the state without the removed task
  };
    ...
    <>
      <div className="main">
        ...
        <div>
          ...
          <h2 className="counting">You have {remainingTask} tasks remaining</h2> {/* count remaining (more than 1 have 's') */}
          {tasks.map((task) => (
            <Task 
              key={task.id} 
              name={task.name}
